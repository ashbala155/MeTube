...
class GridClass
{
    ...
    {
        $query = $this->con->prepare("SELECT * FROM keywords where videoId = '$mediaId'");
        $query->execute();
    ...
    public function createHomeContentBlock($type, $category, $sortby, $loggedInUserName, $hide = 'not hidden')
    {
        if ($loggedInUserName == "") {
            ...
            return $this->createBlock(ucfirst($type."s"), $gridItems);
        } else {
    ...
    public function createSearchContentBlock($type, $keywords, $sortby, $loggedInUserName, $size)
    {
        if ($loggedInUserName == "") {
            ...
            return $this->createBlock(ucfirst($type."s from '".$keywords."'"), $gridItems);
        } else {
    ...
    public function getSuggestions($videoId)
    {
        ...
        //                             and visibility = 'Public' order by rand() limit 4");

        $query = $this->con->prepare("SELECT * FROM media where media.category = '$mediaCategory' and visibility = 'Public' order by rand() limit 4");
        $query->execute();
    ...
    {
        $sqlString = "SELECT media.* FROM media where media.visibility='$privacy' and media.mediaType = '$type'";
        if ($category != "") {
    ...
    public function getPublicItemsUser($type, $category, $sortby, $privacy, $loggedInUserName)
    {
        $sqlString = "SELECT media.* FROM media 
                                    ...
                                    left outer join contacts on users.userName=contacts.userName and contactUserName='$loggedInUserName' 
                                    where media.visibility='$privacy' and media.mediaType = '$type' and (contactStatus!='Blocked' or contactStatus is NULL)";
        if ($category != "") {
            $sqlString .= " and media.category = '$category'";
        }
        if ($sortby != "") {
    ...
    {
        $sqlString = "SELECT media.* FROM media 
                      inner join keywords on media.id = keywords.videoId
                      where media.visibility='$privacy' and media.mediaType = '$type' and keywords.keyword = '$keywords'";
    ...
    public function getKeywordsItemsUser($type, $keywords, $sortby, $privacy, $loggedInUserName, $size)
    {
        $sqlString = "SELECT media.* FROM media 
                      ...
                      inner join users on media.uploadedBy=users.userName and users.userName !='$loggedInUserName' 
                      left outer join contacts on users.userName=contacts.userName and contactUserName='$loggedInUserName'
                      where media.visibility='$privacy' and media.mediaType = '$type' and (contactStatus!='Blocked' or contactStatus is NULL) and keywords.keyword = '$keywords'";
        if ($size != "") {
            if ($size == "0-100K") {
                $sqlString .= "and media.mediaSize < 102400 ";
            } else if ($size == "100K-1000K") {
    ...
    {
        $sqlString = "SELECT media.* FROM media inner join contacts on media.uploadedBy=contacts.userName 
                                where contactUserName='$loggedInUserName' 
                                and media.mediaType = '$type'
                                and ((contactType='Family' and media.visibility='Family') 
                                     or (contactType='Friend' and media.visibility='Friend') 
                                     or (contactType='Fav' and media.visibility='Fav')) 
                                and contactStatus = 'Not Blocked'";
